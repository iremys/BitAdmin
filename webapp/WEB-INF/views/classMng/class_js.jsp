...
//클래스(반) 수정 버튼 클릭시
$("#btn_class_modify").on("click", function(){
	modifyClass();
});
...
function modifyClass(){
	//히든
	var class_id = $("#frm_class input[name='class_id']").val();//클래스(반)코드
	var classCate_id = $("#frm_class input[name='classCate_id']").val();//카테고리코드
	var curri_id = $("#frm_class input[name='curri_id']").val(); //사용한 커리큘럼아이디
	var classCode = $("#frm_class input[name='classCode']").val(); //클래스(반) 운영코드
	var startDate = $("#frm_class input[name='startDate']").val(); //시작일
	var endDate = $("#frm_class input[name='endDate']").val();//종료일
	var maxCnt = $("#frm_class input[name='maxCnt']").val();//정원
	
    //전형일시 고민
    var testDateList =[];
    $(".testTimeArea").each(function(e){
    	$this = $(this);
    	var testDate_id = $this.find("input[name='testDate_id']").val();
    	var testDate = $this.find("input[name='testDate']").val();
    	var testTime = $this.find("select[name='testTime']").val();
    	
    	var testDateVo = {
    		testDate_id: testDate_id,
    		class_id: (class_id<=0)? null : class_id,
    		testDate: (testDate=='')? null : testDate,
    		testTime: testTime
    	}
    	testDateList.push(testDateVo);
    });
    
    
    var lectureTime = $("#frm_class input[name='lectureTime']").val();//강의시간
    var price = $("#frm_class input[name='price']").val();//교육비용
    var support = $("#frm_class select[name='support']").val();//교육비지원
   
    var managerInfo = $("#frm_class input[name='managerInfo']").val();//문의(담당자)
    var pageURL = $("#frm_class input[name='pageURL']").val();//상세페이지링크
   
	var operState = $("#frm_class input:radio[name='operState']:checked").val();//운영상태
	var gisuName = $("#frm_class input[name='gisuName']").val();//기수명
	...
	classVo.testDateList = testDateList;

	console.log("modifyClass()-->", JSON.stringify(classVo));
	$.ajax({
	    url: "${pageContext.request.contextPath }/class/modifyClass",
	    type: "POST",
...
function classRender(classVo){
	console.log("classRender()-->" , classVo);
	formClear("#frm_class");
	$("#frm_class #text_classCateName").text(classVo.cateName); //카테고리명(업무구분)
	
	if(classVo.curri_id > 0){
		$("#frm_class #text_curriName").text(classVo.curriName); //사용한 커리큘럼명
		$("#btn_curri_remove").show();
		$("#frm_class input[name='curri_id']").val(classVo.curri_id); //사용한 커리큘럼아이디
		
	}else{
		$("#text_curriName").empty();
		$("#btn_curri_remove").hide();
		$("#frm_class input[name='curri_id']").val("-1");
	}
	...
	//전형일--->추후 업데이트 
	if(classVo.testDateList.length > 0){
		$("#frm_class input[name='testDate']").val(classVo.testDateList[0].testDate);//전형일
		$("#frm_class select[name='testTime']").val(classVo.testDateList[0].testTime).prop("selected", "selected");//전형시
		$("#frm_class input[name='testDate_id']").val(classVo.testDateList[0].testDate_id);//전형일 코드
	}
...
/***************************************************************/
//자동저장 포커스아웃 이벤트
/* $(".classAutoa").on("focusout", function(){
...
//전형일자 포커스아웃 이벤트
/* $(".testDateAuto1").on("focusout", function(){
...
//전형일 가져오기
function testDateRender(testDateVo){
	
	var html = "" ;
	html += "<div class='pull-left form-inline w240 testTimeArea'>" ;
	html += "	<span class='input-group border-inputcolor w120'>" ;
	html += "		<input type='text' name='testDate' value='"+ testDateVo.testDate +"' class='form-control border-none'  data-select='datepicker' data-toggle='datepicker'  placeholder='YYYY-MM-DD'>" ;
	html += "		<span class='input-group-btn'>" ;
	html += "			<button type='button' class='btn btn-date border-none' data-toggle='datepicker'>" ;
	html += "				<i class='fa fa-calendar'></i>" ;
	html += "			</button>" ;
	html += "		</span>" ;
	html += "	</span>" ;
	html += "	<span>" ;
	html += "		<select class='form-control w80' name='testTime' >" ;
	html += "			<option value=''>--:--</option>" ;
						for(var t=8; t<24 ; t++){
							if(t<10){
								var tStr = "0" + t + ":00";
								console.log(tStr);
								if(tStr == testDateVo.testTime){
									html += "			<option value='"+tStr+"' selected>"+tStr+"</option>" ;
								}else{
	...
	html += "		</select>" ;
	html += "	</span>" ;
	html += "	<input type='text' name='testDate_id' value='"+ testDateVo.testDate_id +"'>" ;
	html += "	<span class='label label-danger mouse btn_testDate_remove'>X</span>" ;
	html += "</div>" ;
	html += "" ;
	$("#testTimeTd").append(html);
}



function testDateAddRender(){
	var html = "" ;
	html += "<div class='pull-left form-inline w240 testTimeArea'>" ;
	html += "	<span class='input-group border-inputcolor w120'>" ;
	html += "		<input type='text' name='testDate' value='' class='form-control border-none datep'  data-select='datepicker' data-toggle='datepicker'  placeholder='YYYY-MM-DD'>" ;
	html += "		<span class='input-group-btn'>" ;
	html += "			<button type='button' class='btn btn-date border-none datep'  data-select='datepicker' data-toggle='datepicker'>" ;
	html += "				<i class='fa fa-calendar'></i>" ;
	html += "			</button>" ;
	html += "		</span>" ;
	html += "	</span>" ;
	...
	html += "		</select>" ;
	html += "	</span>" ;
	html += "	<input type='text' name='testDate_id' value='-1'>" ;
	html += "	<span class='label label-danger mouse btn_testDate_remove'>X</span>" ;
	html += "</div>" ;
	html += "" ;
